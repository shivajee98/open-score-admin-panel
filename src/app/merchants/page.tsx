...
import { Search, Plus, Trash2, Ban, CheckCircle, MoreVertical, ReceiptIndianRupee, CheckSquare, Square, Save, Eye, Clock, X, Check, ChevronLeft, ChevronRight, Download, AlertTriangle, ArrowRightLeft, MapPin, Filter, Calendar, Users as UsersIcon, ShieldAlert, ShieldCheck, BadgeCheck, Unlink, AlertCircle, Phone, Video, Link as LinkIcon, Edit, Send, Mail, Bell } from 'lucide-react';
...
const UserRow = ({ user, selectedIds, toggleSelect, toggleStatus, handleDelete, setSelectedUser, setIsCreditsModalOpen, reloadUsers, currentUser, onReviewKyc, onVaultConfig, onSendEmail, onSendNotification }: any) => {
             ...
             <td className="p-6">
                <div className="flex flex-col gap-1">
                    ...
                    <div className="mt-2 flex flex-col gap-1">
                        ...
                        {(user.aadhar_image || user.aadhar_back_image || user.pan_image || user.electricity_bill || user.shop_rent_doc) && (
                            <div className="flex gap-1 mt-1">
                                ...
                                {user.electricity_bill && <a href={user.electricity_bill} target="_blank" className="w-6 h-4 bg-blue-50 rounded border border-blue-100 flex items-center justify-center text-[8px] font-bold text-blue-400 hover:text-blue-600 transition-colors" title="Electricity Bill">E</a>}
                                {user.shop_rent_doc && <a href={user.shop_rent_doc} target="_blank" className="w-6 h-4 bg-indigo-50 rounded border border-indigo-100 flex items-center justify-center text-[8px] font-bold text-indigo-400 hover:text-indigo-600 transition-colors" title="Rent/Shop Doc">R</a>}
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="flex flex-1 gap-2">
                        ...
                        <div className="flex items-center gap-3 px-4">
                            ...
                            <div className="flex items-center gap-2 px-4 py-3 bg-indigo-50/50 border border-indigo-100/50 rounded-xl text-indigo-700 text-sm font-bold shadow-sm whitespace-nowrap">
                                ...
                                <span>{pagination.total || 0} Merchants</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50/50">
                            <tr>
                                ...
                                <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest">Docs</th>
                                <th className="p-6 text-xs font-bold text-slate-400 uppercase tracking-widest">QR</th>
            ...
            {isAdmin && isSupportModalOpen && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95">
                        ...
                        <p className="text-slate-500 font-medium mb-6">Set a dedicated support number for <span className="text-slate-900 font-bold">{selectedIds.length} merchants</span>.</p>
                        ...
                        <form onSubmit={handleBulkSupportUpdate}>
                            ...
                            <button
                                type="button"
                                onClick={async () => {
                                    if(!confirm('Are you sure you want to clear and hide support numbers for all selected merchants?')) return;
                                    setActionLoading(true);
                                    try {
                                        await apiFetch('/admin/users/bulk-support', {
                                            method: 'POST',
            ...
            {isAdmin && isMeetingModalOpen && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95">
                        ...
                        <p className="text-slate-500 font-medium mb-6">Set a meeting link for <span className="text-slate-900 font-bold">{selectedIds.length} selected merchants</span>.</p>
                        ...
                        <button
                            type="button"
                            onClick={async () => {
                                if(!confirm('Are you sure you want to clear and hide meeting links for all selected merchants?')) return;
                                setActionLoading(true);
                                try {
                                    await apiFetch('/admin/users/bulk-meeting', {
                                        method: 'POST',
